...
class DeepSeekLLMService(BaseLLMService):
    """Streams tokens from DeepSeek models via OpenAI-compatible API.
    ...
    default_model = "deepseek-chat"  # fallback if DEEPSEEK_MODEL is unset
    ...
    async def stream(
        ...
        try:
            ...
            logger.error("DeepSeekLLMService timeout: %s", exc)
            raise LLMError(f"DeepSeek timeout: {exc}") from exc
        except APIError as exc:
            logger.error("DeepSeekLLMService API error: %s", exc)
            raise LLMError(f"DeepSeek API error: {exc}") from exc
        except Exception as exc:
            logger.error("DeepSeekLLMService unexpected error: %s", exc)
            raise LLMError(f"DeepSeek error: {exc}") from exc
